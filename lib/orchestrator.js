 ...
 * The cache is written atomically (tmp + rename, same pattern as state.json)
 * so a kill between write and rename never leaves a half-written cache. */
async function computeAndCacheRankings() {
  ...
  const tmp = `${rankingsFile()}.tmp`;
  try {
    await fs.writeFile(tmp, `${JSON.stringify(result, null, 2)}\n`, 'utf8');
    await fs.rename(tmp, rankingsFile());
  } catch (err) {
    await fs.rm(tmp, { force: true }).catch(() => {}); // best-effort tmp cleanup
    throw err;
  }
  return result;
}
...
/**
 * Empty rankings payload with the same shape as computeRankings output — the
 * dashboard renders empty arrays as "아직 데이터가 없습니다", so a failed
 * computation with no cache serves 200 with empty tables instead of a 500.
 */
function emptyRankings(now = new Date()) {
  const nowMs = now.getTime();
  const windows = {
    daily: { start: todayStartKst(now).getTime(), end: nowMs }, // KST today 00:00 ~ now
  ...
  for (const metric of ['posts', 'comments', 'recv', 'given']) {
    top[metric] = { daily: [], weekly: [], monthly: [] };
  }
  return {
 ...
 * the last good data). When there is no cache either, return an empty result
 * instead of throwing — the dashboard shows empty tables rather than a 500
 * error banner.
 */
async function getRankings() {
  try {
    ...
    console.warn(`[orchestrator] rankings computation failed (${err.message}); serving empty result`);
