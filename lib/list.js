...
/**
 * Crawl the board list from page 1 (or startPage on resume), appending
 * article events and updating state.scanned. Stop rules (checked after each
 * page is processed):
 ...
 * onProgress({page, total}) is awaited after each page (orchestrator persists
 * progress.listPage so a killed run resumes from the next page). onError is
 * called for page fetch failures (falls back to onWarn).
 * Returns {total, pages, fetchQueue, voteQueue}.
 */
async function crawlList({ windowStart, onWarn, onError, onProgress, maxPages = config.maxPages, startPage = 1 } = {}) {
  const state = await store.loadState();
  const scanned = new Set(state.scanned || []);
  const fetchQueue = [];
  const voteQueue = [];
  const now = new Date();
  let page = startPage; // site is 1-indexed; page=0 aliases to page=1 (verified live)
  let processed = 0;
  let emptyStreak = 0;
  let total = 0;
  ...
  while (page <= maxPages) {
    ...
    try {
      ...
      (onError ?? onWarn)?.(`list page ${page} fetch failed: ${err.message}`);
      break;
    }
  ...
  // Merge into the CURRENT state so progress saved during the loop survives.
  const cur = await store.loadState();
  await store.saveState({ ...cur, scanned: [...scanned] });
