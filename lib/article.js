...
async function loadArticleMap() {
  ...
  for (const line of text.split('\n')) {
    ...
    try {
      ...
      if (ev.t === 'article') map.set(ev.idx, { badge: ev.badge ?? 0, ts: ev.ts, nick: ev.nick, recv: ev.recv ?? 0 });
    } catch {
 ...
 * pagesFetched, titleCnt}] }. `now` is injectable so relative-time parsing is
 * deterministic in tests (defaults to the real clock).
 */
async function crawlArticles({ fetchQueue, articleMap, onWarn, onError, now = new Date() } = {}) {
...
export { parseArticlePage, parseComments, parseCommentLi, extractNick, crawlArticles, fetchArticle, loadArticleMap };